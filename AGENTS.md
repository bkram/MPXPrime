...
- Branch model: `main` is the default branch and tracks released versions. `develop` is the long-lived integration branch where unreleased work accumulates. Feature work either commits directly on `develop` or branches off `develop` and merges back. Releases ship by merging `develop` into `main`, tagging `v<version>` from `main`, and pushing the tag — which triggers `.github/workflows/release.yml`, runs `./build-release.sh <version>`, and publishes the resulting DMG as a GitHub Release. (Pre-0.21 the layout was per-release `develop/v.NN` branches with no `main`; flipped to the `main` + `develop` model alongside v0.21.)
