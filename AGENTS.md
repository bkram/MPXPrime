...
Runtime code is split across seven SPM targets (see the target list under "Project" above): the `MPXPrime` encoder executable, the `MPXPrimeMeter` analyzer executable, the shared `MPXPrimeCore` (DSP + `MPXDecoder` + `RDSStreamDecoder` + the Meter measurement engine) and `MPXPrimeUI` (Canvas components) libraries, `MPXPrimeNative` (FTZ/DAZ C shim), `MPXPrimeRecording` (WAV writer/recorder), and `CMPXTuner` (vendored RTL-SDR/SDRplay tuner). Key files:
