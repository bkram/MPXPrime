...
| Receiver-model verification | Landed in 0.27: reusable `MPXDecoder` (pilot-PLL or referenced 38 kHz, deemphasis, noise gate, stereo-collapse cooldown) used both by the monitor path and by the new `--verify-receiver` CLI mode. Reports stereo separation at 1 / 10 / 14 kHz, mono compatibility, pilot percent + phase, and RDS lower/upper/center sideband levels. | Remaining roadmap work is promoting per-scenario receiver thresholds from TIGHT to hard failure once defaults are tuned, plus extending to the preset sweep. |
| Anti-aliased clipping (US 6,937,912) | Phase A landed in 0.27: `BandLimitedStep` BLEP/BLAMP primitive + spectral gate (`AntiAliasedClipperProbeTests`). Phase B candidate: `AcceleratedBandlimitedResidualClipper` (vDSP-accelerated residual-bandlimiting) wired as opt-in inner kernel of `OversampledPeakLimiter` / `StereoLinkedOversampledPeakLimiter` behind `pre_encode_bandlimited_residual_enabled`. Probe shows 13.84 dB cleaner alias-bin energy vs hard clip on the 5.111 kHz / 48 kHz stress probe, fundamental preserved. | Remaining 0.27 work: A/B program-material listening, multi-tone IM gates, then default the residual path on; Phase C: apply same primitive to `audioCompositeSoftClipEnabled` shaper inside `processFinalComposite`. |
