...
| Anti-aliased clipping (US 6,937,912) | Phase A + Phase B candidate landed in 0.27: `BandLimitedStep` + `AcceleratedBandlimitedResidualClipper` opt-in kernel behind `pre_encode_bandlimited_residual_enabled`, operator-tunable taps + cutoff, parameter-sweep-validated defaults, full-MPX-chain A/B + CPU-cost regression coverage. | A/B program-material listening with the toggle on, decide whether to default on for ship; Phase C: apply same primitive to `audioCompositeSoftClipEnabled` shaper. |
| Transient-aware multiband | Not landed for `MonoCompressor` (PrimeBass and wideband AGC have their own detectors). | Valid DSP quality upgrade — see roadmap #2 below. |
| Inter-band gain coupling | Not landed. Current multiband linking is L/R sidechain linking inside each band. | See roadmap #3 below. |
| Multiband composite clipping | Not landed. Current `CompositeClipper` is fullband. | See roadmap #4 below. Higher effort. |
| Dynamic pre-emphasis load manager | Not landed. Placement is correct in L/R upstream of pre-encode limiter; no HF sidechain relaxation. | See roadmap #5 below. |
| Composite peak-to-RMS governor | Not landed. `BS412PowerLimiter` controls regulatory average MPX power, not crest/peak-to-RMS density. | See roadmap #7 below. |
...
### 1. Transient-aware multiband envelope follower
...
Already used or structurally equivalent (do not re-propose; see plan.md "Patent-backed improvements backlog → Already implemented" for code pointers): US6337999B1 (oversampled differential clipper), US4460871A / US4249042A (Orban overshoot/cross-coupled compressor — partial via per-band cancellation), US6618486B2 (BS.412 multiplex power controller), and the PrimeBass set (US5930373A, US4150253A, US5424488A, US5359665A).
...
- **Multiband composite limiting.** Per-band clip thresholds give more loudness for the same peak modulation and avoid the "tonal shift under heavy clipping" Orban Optimod 8x00 is known for fixing. (Same item as roadmap #3 above.)
