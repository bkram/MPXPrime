...
`--verify-program-ab <file-or-dir> [--ab-profile music_clean] [--ab-csv out.csv] [--seconds 30]` (macOS only) is the REAL-MUSIC A/B for the Advanced Dynamics campaign. It decodes your audio files (wav/aiff/mp3/m4a/flac/caf), renders each excerpt through the chosen Format Profile twice -- (A) the shipped AGC + multiband chain and (B) the same profile with Advanced Dynamics -- and measures both composites with the Meter's own measurement engine, so the numbers line up with the live MPX Prime Meter: BS.412 MPX power, max/average deviation, positive/negative peaks, SM.1268 exceedance, plus decoded-audio crest, >6 kHz crest, stereo image, band balance, the leveler's per-band gain range, and a pumping index (0.5-4.5 Hz modulation of the B-vs-A loudness envelope ratio). Identical input per chain makes every delta deterministic and re-runnable. RDS is off during the render (its wall-clock text scheduler would make renders non-reproducible; the constant-amplitude subcarrier is identical in both chains anyway). The thresholds were calibrated and armed 2026-09-04 on a captured operator corpus (any tripped bound = exit 1 TIGHT; a composite-budget violation = exit 3 FAIL). The pumping index aligns the two chains' envelopes first (the leveler carries ~10 ms more group delay) and deliberately conflates leveling activity with pumping -- its bound catches a runaway, listening judges the character.
