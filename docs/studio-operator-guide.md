...
Two readouts, two domains: the **deviation** meter is a modulation-domain figure -- `output_gain_db` and `mpx_line_output_dbfs` are divided back out, so it reads the same kHz regardless of how the exciter drive is trimmed (before 0.50 it under-read by exactly the output trim), and its kHz scale is fixed -- composite amplitude 1.0 is 75 kHz at any deviation setting, so a 50 kHz station reads 50 kHz at full modulation (before 0.60 it read 33 kHz there). **DAC Peak** is the electrical figure -- the level actually presented to the converter, post both trims. Calibrate deviation at the exciter; watch DAC Peak to know how hot the wire is.
...
- `BS.412` (`Sound` -> `BS.412`): only required if you operate under EU power-limiting rules (a cap on average multiplex power over any 60 seconds). Outside that regulatory context, leave `Enable BS.412` off -- it actively pulls level back over long windows and dulls dynamics. When you do need it, the one control is `Ceiling`, in **dBr**: **0 dBr is the limit the Recommendation sets**, so 0.0 is the setting for compliance and a negative value is a safety margin below it. It measures the whole signal you transmit, pilot and RDS included, and it keeps measuring even when the limiter is switched off, so turning it on acts immediately instead of waiting a minute. Two readouts on the Monitoring page tell you where you are: **MPX Power** shows the figure in dBr once a full minute has been observed, and shows `42 / 60 s` while it is still filling. **BS.412 GR** shows how much it is holding the audio down. While the Test Tone is running the limiter steps aside so the tone keeps its calibrated level, and the readout says `suspended (tone)` -- do not read that as compliant. If it says `pilot/RDS over budget`, your pilot and RDS injection alone exceed the ceiling and no amount of audio reduction can fix it; lower the injection or raise the ceiling. Before 0.60 this control was labelled `Threshold` and read in a different unit that did not correspond to the standard; an upgraded configuration adopts the standard's 0 dBr ceiling.
