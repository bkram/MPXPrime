...
private func advancedDynamicsComparisonScenarios() -> [VerificationScenario] {
...
private func runAdvancedDynamicsComparison(
    baseConfig: AppConfig,
    durationSeconds: Double
) -> Int32 {
...
private func advancedDynamicsIdempotencyDeltaDB(config: AppConfig) -> Float {
...
private struct HFTransientScenario {
    let name: String
...
private func hfTransientScenarios() -> [HFTransientScenario] {
...
private struct HFTransientMeasurement {
...
private func hannPowerSpectrum(_ samples: [Float], start: Int, fftSize: Int) -> [Float] {
    let halfSize = fftSize / 2
...
private func hfCrestDB(_ samples: [Float], start: Int, count: Int, sampleRate: Float) -> Float {
    var hp = Biquad()
    hp.configureHighpass(cutoffHz: 6_000.0, sampleRate: sampleRate)
...
private func measureHFTransient(
    config: AppConfig,
...
private struct HFTransientChainVariant {
    let label: String
...
private func applyFieldChain2026_08(_ c: inout AppConfig) {
    c.inputGainDB = -4.0
...
private func hfTransientChainVariants() -> [HFTransientChainVariant] {
...
private let hfTransientMinRideSINADDB: Float = 30.0
private let hfTransientMinHatSINADDB: Float = 20.0
private let hfTransientMinHatSINADLoudDB: Float = 15.0
private let hfTransientMinCrestDeltaDB: Float = -6.0
private let hfTransientMaxGapSpillDB: Float = -36.0

private func runHFTransientVerification(
    baseConfig: AppConfig,
    durationSeconds: Double
) -> Int32 {
