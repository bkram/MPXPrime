...
// Opt-in benchmark suite. Captures absolute per-stage and per-rate
// DSP cost on the current chain so the dual-rate refactor (planned 0.31)
// has a concrete "before" reference to compare against.
//
// Gated on the MPXPRIME_BENCH env var so it does not run on every
// `swift test`. Run with release optimizations:
//
//   DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
//     MPXPRIME_BENCH=1 swift test -c release --package-path macOS --filter Benchmark
//
// Save the stdout output as the "before dual-rate" baseline. Re-run
// after the refactor and diff. The summary line at the bottom estimates
// what the dual-rate architecture should save, derived from the
// per-stage breakdown.
...
struct BenchmarkSuite {
    ...
    //
    // Audio-domain stages process L/R below the stereo encoder. In the
    // planned dual-rate architecture these stages move to 48 kHz, so
    // their cost should scale down roughly with the rate ratio. MPX-
    // domain stages process the composite at the high rate and stay where
    // they are.
    ...
    /// oversampling factor. Quantifies the cost knob exposed to the
    /// operator by the UI picker.
    ...
    private func header() -> String {
        ...
        lines.append("Machine: \(sysctl("hw.model")) / \(sysctl("machdep.cpu.brand_string"))")
    ...
    /// Per-stage cost as the delta when the stage is enabled vs disabled,
    /// all other stages held at the full-chain defaults at 192 kHz with
    /// RDS + stereo enabled. Median of 3.
    private func perStageSection() -> String {
        ...
        for probe in Self.stageProbes {
            ...
            // Negative delta = disabling actually made it slower (within
            // measurement noise). Show as 0.0 and flag in the table for honesty.
    ...
    private func summarySection() -> String {
        ...
        // Estimate dual-rate cost: audio-domain stages at 1/4 cost (48k vs
        // 192k), MPX-domain stages unchanged, plus ~5% resampler overhead.
