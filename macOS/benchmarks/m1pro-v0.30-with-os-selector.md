...
- Dual-rate refactor (docs/project-roadmap.md "Next up" #1) projection is unchanged: composite clipper stays at MPX rate regardless of audio-domain rate, so its 14.07% RT cost survives the refactor. The combination dual-rate + 32x clipper lands at roughly 24.85% RT (post dual-rate baseline) + 14 pp (32x premium over 16x) = ~39% RT -- comparable to today's 16x cost, just with the spec-sheet 32x defended.
