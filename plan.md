...
| **P8 — PHASE 1 LANDED / VALIDATED, PHASE 2 NEXT** | [US 5,579,404](https://patents.google.com/patent/US5579404A/en) / [EP 0685130 B1](https://patents.google.com/patent/EP0685130B1/en) | Digital audio limiter — subband-aware look-ahead | Expired ~2013-11 (US) / 2014-02 (EP) | `StereoLinkedOversampledPeakLimiter` (pre-encode L/R) | Phase 1 (LANDED, 0.30, default 1.0 ms): textbook delay+detector look-ahead on the existing stereo-linked limiter (US 4,208,548 prior art, no patent citation needed). Real-program listening A/B confirmed audibly cleaner HF transients and improved HF reach on dense / sibilant content. Phase 2 (NEXT): the Dolby split-band variant — apply the lookahead detector only to HF content above ~3-5 kHz. Maps cleanly onto MPX Prime's situation: pre-emphasis adds +10-12 dB specifically to HF, so the peaks the pre-encode limiter fights are concentrated above ~3 kHz. Targeting the lookahead at HF avoids wasting attack ramp on LF content that doesn't need it, and leaves LF perceived loudness / punch untouched. Assignee: Dolby Laboratories Licensing Corp. |
...
**Phase 2 — HF-subband-aware look-ahead per P8 / [US 5,579,404](https://patents.google.com/patent/US5579404A/en) (Dolby, expired 2013). NEXT.** Split the detector path into a HF-only band (high-pass at ~3-5 kHz on a copy of the input feeding the detector; audio path stays full-band). The look-ahead engages only when HF transients are detected, leaving LF perceived loudness / punch untouched. Architecturally clean answer to "where in the spectrum is the pre-encode limiter actually working hardest?" Justified by Phase 1's HF-reach improvement — focusing the detector on the band that actually triggers the lookahead path should push the same effect further without affecting LF.

Scope: Phase 2 ~2-3 days including subband detection (1-pole or 2-pole biquad HP, ~4 kHz cutoff), opt-in `pre_encode_lookahead_hf_only` flag, listening A/B, tests verifying LF-only transients don't trigger limiting while HF transients still do.
