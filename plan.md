...
| **P8 — LANDED, BOTH PHASES DEFAULT-ON** | [US 5,579,404](https://patents.google.com/patent/US5579404A/en) / [EP 0685130 B1](https://patents.google.com/patent/EP0685130B1/en) | Digital audio limiter — subband-aware look-ahead | Expired ~2013-11 (US) / 2014-02 (EP) | `StereoLinkedOversampledPeakLimiter` (pre-encode L/R) | Phase 1 (LANDED 0.30, default 1.0 ms): textbook delay+detector look-ahead. US 4,208,548 prior art, no patent citation needed. Real-program listening A/B confirmed audibly cleaner HF transients and improved HF reach. Phase 2 (LANDED 0.30, default-on): the Dolby split-band variant — detector path high-passed at 4 kHz so look-ahead engages only on HF content. Pre-emphasis adds +10-12 dB to HF before the limiter, so the peaks the limiter actually fights are concentrated above ~3 kHz. Targeting the lookahead at HF leaves LF perceived loudness / punch untouched while preserving Phase 1's HF-reach improvement. Assignee: Dolby Laboratories Licensing Corp. |
...
3. **Preset tuning — LANDED in 0.30 as Format Profiles.** Eight format-first profiles (Community Radio / Pop · AC / CHR · Top 40 / Rock / EDM · Dance / Urban · Hip-Hop / Jazz · Classical / News · Talk) atomically apply multiband + final-stage + PrimeBass + widener + composite-clipper bundles via a top-of-Processing-Overview picker (`format_profile_id` INI key, `MPXPrimeViewModel.FormatProfile` struct, `applyFormatProfile(_:)`). Default `community_radio` matches the prior shipping defaults — regression-guarded. INI-fragment approach (separate `.ini` files alongside the binary) was the original wording here but the hardcoded eight-profile set covers the same audience with cleaner UX; revisit fragments only if operators want shareable text files. Remaining: real-program listening A/B per profile to fine-tune the clipper drives (separate validation pass, like the look-ahead one).
...
**Phase 1 — basic look-ahead. LANDED in 0.30, default 1.0 ms.** Reused the `LookaheadLimiter` pattern (delay-line + un-delayed detector). Added audio-rate delay buffers to `StereoLinkedOversampledPeakLimiter`, a `lookaheadMS` configure parameter (clamp 0-5 ms), and a "future peak hint" into `stereoStep` so the detector uses `max(osPeak, futurePeakHint)`. Restart-only (allocates delay buffer); silent live-resize is a separate hardening pass like the composite clipper got in 0.27. Tests in `PreEncodeLookaheadTests`: lookahead=0 bit-identical (regression guard), lookahead>0 reduces peak overshoot, mono-link discipline preserved, live-apply preserves lookahead. Real-program listening A/B confirmed audibly cleaner HF transients and improved HF reach. Patent risk: none — delay+detector is pre-1980 prior art ([US 4,208,548](https://patents.google.com/patent/US4208548A/en), expired ~1997).

**Phase 2 — HF-subband-aware look-ahead per P8 / [US 5,579,404](https://patents.google.com/patent/US5579404A/en) (Dolby, expired 2013). LANDED in 0.30, default-on.** Detector path runs through a 2nd-order Butterworth high-pass biquad (`pre_encode_lookahead_hf_cutoff_hz`, default 4 kHz, clamp 1-12 kHz). When `pre_encode_lookahead_hf_only` is True (default), the lookahead engages only on HF transients above the cutoff. Audio path stays full-band — only the gain envelope is affected. Tests in `PreEncodeLookaheadTests`: LF transients produce measurably less GR with HF-only on; HF transients still trigger GR within ~1 dB of broadband; HF-only=false is bit-identical to Phase 1. Both Phase 1 and Phase 2 ship default-on in 0.30 based on Phase 1 listening A/B (HF reach improvement) — Phase 2 should preserve that win and add LF punch protection.
