...
| -------- | ---------------------------------------------------------------- | ------------------------------------------------------ | ---------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **P0 — SCHEDULED v0.27** | [US 6,937,912](https://patents.google.com/patent/US6937912B1/en) | Anti-aliased clipping with band-limited step functions | 2025-09          | `OversampledPeakLimiter` (pre-encode L/R) + `audioCompositeSoftClipEnabled` shaper | Replaces the inner clip kernel: when a peak crosses threshold, substitute a band-limited polyBLEP-style step matched to the host filter's group delay. Stops IM from being generated rather than relying on decimation to remove it. Both stages are engaged in the verifier scenarios — this is the patent that moves the `>67k/in` and `>60k/in` numbers. **Scheduled as "Next up" #1 — see Next up section for phase breakdown.** |
...
1. **Anti-aliased clipping kernel — US 6,937,912 (Orban, expired Sept 2025).** Headline DSP work for v0.27. Replace the inner clip kernel in `OversampledPeakLimiter` (pre-encode L/R, 4× OS) and the `audioCompositeSoftClipEnabled` shaper with band-limited step functions (polyBLEP / BLAMP-style correction) matched to the host LP filter's group delay. The current chain relies on oversampling + Kaiser-windowed FIR decimation to remove alias products generated by the soft-clip's derivative discontinuities; this leaves a tail visible in `bright_dense` (`>60k/in -40.8 dB`, `>67k/in -42.0 dB` — both flagged TIGHT today). The patent's technique stops aliasing at the source rather than filtering it out after the fact: detect threshold crossings with sub-sample accuracy, splice in a precomputed band-limited step waveform spanning a few samples around the crossing. Both stages are engaged in the verifier scenarios, so this is the single change that moves the `>67k/in` / `>60k/in` verifier numbers and clears the residual TIGHT classification on `hf_edge_12k` + `bright_dense`. Scope: ~2–3 weeks across four phases:
   - **A.** `BandLimitedStep` primitive — pick between polyBLEP polynomial (cheap, ~4-sample window) and table-lookup BLEP (more accurate, ~16–32 sample window). Stand-alone helper with allocation-free hot path.
   - **B.** Apply to `OversampledPeakLimiter` (pre-encode); A/B against current oversampled+decimated path with `--verify --baseline-strict`.
   - **C.** Apply to `softClipSafety` calls inside `processFinalComposite` (audio-composite shaper).
   - **D.** Verifier baseline refresh; expected `>60k/in` and `>67k/in` improvement 8–15 dB; listening confirm on real program.
...
|---|---|---|---|---|
| **1 — scheduled v0.27** | Anti-aliased clipping kernel (US 6,937,912) | "Next up" #1; `OversampledPeakLimiter` + audio-composite shaper | 2–3 weeks | **Highest** — clears the residual `>60k/in` / `>67k/in` TIGHT classification; stops IM at the source instead of filtering after the fact |
