...
Audit done 2026-05-09 against published Orban Optimod 8500/8600, Omnia.9/.11, and Stereo Tool architectures. The current chain matches industry practice on every load-bearing position — phase rotator early, AGC before EQ, multiband with per-band limiters, encoder lowpass before stereo encoding, composite clipper before BS.412, and pilot/RDS injection post-clipper at constant amplitude (the professional invariant). Three deviations from canonical ordering exist; listed here so we can decide whether and when to address them rather than dismissing them.

| Deviation | Industry | Ours | Audible cost | Effort | Recommendation |
|---|---|---|---|---|---|
| **PrimeBass before multiband** | Bass enhancers (MaxxBass / Aural Exciter / Big Bottom) typically post-multiband or in parallel insert/return; multiband then doesn't compress synthesised harmonics back down. | Stage 8, before multiband (#10). Werrbach transient detector in Phase 2 partially compensates. | Marginal — maybe 0.5 dB more LF peak headroom, slightly tighter bass on percussive material. | ½ day to feature-flag + retune dual-envelope time constants for the post-multiband source. | **Worth A/B-testing.** Cheap to investigate; cheap to revert if it doesn't sound better. |
| **Stereo widener before multiband** | Optimod 8x00 puts widener post-multiband or in M/S after stereo encoding. Multiband on a widened L/R can over-enhance side-channel differences in HF bands where compression ratios are highest. | Stage 9, before multiband (#10). | Subtle — slightly more open stereo image on stereo-dense program; no audible difference on mono-leaning material. | ½ day, same A/B pattern. | **Skip unless touching the area.** Improvement is marginal; risk of subtly weirding the stereo image isn't worth it for tonal placement alone. |
| **Pre-emphasis in M/S after pre-encode L/R limiter** | Canonical: pre-emphasis on L/R *before* the L/R limiter — limiter catches the up-to-17 dB HF gain pre-emphasis adds. | M/S inside `makeCompositeComponents` (#17), after pre-encode L/R limiter (#16). HF peaks created by pre-emphasis aren't bounded at the L/R level — composite clipper (#19) handles them. Documented constraint at item 7.6 below; protected by `DSPThroughputTests.preEmphasisDoesNotExplodeFullChainCost` against accidental relocation. | Real but bounded — composite clipper drive has to be set conservatively because pre-emphasis is adding fresh HF peaks late in the chain. Moving pre-emphasis upstream would let the L/R limiter handle them surgically and let composite-clipper drive push harder. ~1–2 dB more loudness on HF-heavy program achievable. | High — last attempt (`b806053`) blew the real-time budget. Need either a cheaper L/R pre-emphasis filter (single biquad per channel = 4 ops/sample, well within budget — but would change the magnitude response shape) or a restructured `OversampledPeakLimiter` that doesn't oversample on the now-emphasized HF. 1–3 days investigation + 2–5 days execution. | **Defer to a future major release.** This is the headline-feature-class change, not a side project. Treat as the structural item for an eventual 0.30. |

**Net realistic improvement** if all three landed cleanly: ~1–2 dB perceived loudness on hard program, subtle tonal cleanups elsewhere. Comparable in magnitude to one PrimeBass-Phase-2-class addition, not to the 0.20 differential-topology jump. The bigger remaining wins are still in Composite-clipper-improvements #2 (look-ahead peak control) and Multiband-DSP-modernisation Phase 2 (transient-aware attack) — both score higher on improvement-per-effort than reordering.
...
See "Next up" #5. **Constraint:** must stay in M/S domain inside `makeCompositeComponents`, or implement as a dedicated sidechain feed into the pre-encode limiter. Moving the audio-path pre-emphasis upstream of the limiter (the `b806053` pattern) is verified to cause ring-overflow dropouts and is now guarded by `DSPThroughputTests.preEmphasisDoesNotExplodeFullChainCost`. The chain-order audit (under "Broadcast-tier follow-ups → Chain-order audit") flags the M/S-after-limiter placement as a deviation from canonical ordering with ~1–2 dB loudness left on the table; revisiting that constraint is its own substantial scope (cheaper L/R pre-emphasis filter or restructured `OversampledPeakLimiter`) and is the headline-feature-class change for a future major release rather than a side project.
