...
**Release-blocking:** **Test tone generator is broken (user report 2026-08-29): "way too loud or not working as intended".** Reproduce with `source_mode = tone` / the Test Tone card at `test_tone_level_db = -20`: the tone enters the full chain, so the AGC lifts a -20 dBFS tone by up to `wideband_agc_max_gain_db` toward the -16 dBFS target and `final_drive_db` then drives it into the composite clipper -- a steady tone at that point IS full deviation, which is probably the "way too loud". Decide the intended semantics (a calibration tone should bypass AGC/multiband/clipper and set deviation directly from `test_tone_level_db`, the way the verifier's `mono_1khz` reference is read) and pin it with a test (tone level in -> composite deviation out); also check the GUI Test Tone card's mode/frequency/level bindings actually reach `RuntimeConfig` live. Then: smoke-test live-apply vs restart-required; tune composite clipper defaults (drive / ceiling / `mpx_clipper_lookahead_ms`) so a fresh install audibly beats `mpxgen` / PiFmRds untweaked.
